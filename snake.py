#!/usr/bin/env python3
"""
Multiplayer Snake — terminal edition (2 players, same keyboard)
...
- Crashing into a wall, yourself, or the other snake kills you.
- Head-on collision between snakes kills both.
...
def main(stdscr):
    ...
    occupied = set(p1["body"]) | set(p2["body"])
    ...
    while running:
        ...
        status = f" P1 (WASD) score: {p1['score']}{' [DEAD]' if not p1['alive'] else ''}   |   P2 (Arrows) score: {p2['score']}{' [DEAD]' if not p2['alive'] else ''}   |   q: quit "
        try:
